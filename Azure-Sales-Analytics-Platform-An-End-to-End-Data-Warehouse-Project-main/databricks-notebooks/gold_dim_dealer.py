...
df_src=spark.sql('''
select distinct(Dealer_ID) as Dealer_ID,DealerName from parquet.`abfss://silver@cargauthadatalake.dfs.core.windows.net/carsales`
''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    ...
    df_sink=spark.sql('''
    select 1 as  dim_dealer_key,Dealer_ID,DealerName from parquet.`abfss://silver@cargauthadatalake.dfs.core.windows.net/carsales` where 1=0
    ''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_tbl=DeltaTable.forPath(spark,'abfss://gold@cargauthadatalake.dfs.core.windows.net/dim_dealer')
    ...
    df_final.write.format('delta')\
        .mode('overwrite')\
            .option('path','abfss://gold@cargauthadatalake.dfs.core.windows.net/dim_dealer')\
